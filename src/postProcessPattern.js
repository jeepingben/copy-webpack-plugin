    ...
    return readFile(inputFileSystem, file.absoluteFrom)
      ...
      .then((content) => {
        ...
        if (pattern.copyPermissions) {
            logger.info(`saving permissions for '${file.absoluteFrom}'`);

            written[targetPath].copyPermissions = pattern.copyPermissions;
            written[targetPath].webpackTo = file.webpackTo;
            logger.info(`saved permissions for '${file.absoluteFrom}'`);

            let constsfrom = inputFileSystem.constants || constants;

            perms |= stats.mode & constsfrom.S_IRWXU;
            perms |= stats.mode & constsfrom.S_IRWXG;
            perms |= stats.mode & constsfrom.S_IRWXO;

            written[targetPath].perms = perms;
        }
